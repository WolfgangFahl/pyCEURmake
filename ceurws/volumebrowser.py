...
from jpwidgets.bt5widgets import About, Alert, App, IconButton, ProgressBar, Switch
...
class Display:
    ...
    def createVolumeSpan(self, a, volume: Volume):
        ...
        wdSpan = jp.Span(a=a)
        jp.Link(
            a=wdSpan, href=f"/volume/{volume.number}", text=f"{volume}:{volume.acronym}"
        )
    ...
    def createEventProceedingsList(
        ...
        li = jp.Li(
            a=ul, classes="list-group-item", text=f"Vol-{getattr(volume, 'number')}"
        )
        ...
        li3 = jp.Li(
            ...
            text=wdSync.getEventNameFromTitle(getattr(volume, "title")),
        )
...
class VolumeListRefresh(Display):
    ...
    async def onRefreshButtonClick(self, _msg):
        ...
        try:
            ...
            ) = self.app.wdSync.getRecentlyAddedVolumeList()
...
class WikidataRangeImport(Display):
    ...
    def __init__(self, app, a):
        ...
        self.fromInput = self.app.createInput(
            labelText="from", placeholder="from", change=self.onChangeFrom, a=self.colA1
        )
        self.toValue = self.app.createInput(
            labelText="to", placeholder="to", change=self.onChangeTo, a=self.colA2
        )
        self.uploadButton = IconButton(
            a=self.colA3,
            iconName="upload-multiple",
            click=self.onUploadButtonClick,
            classes="btn btn-primary btn-sm col-1",
        )
        self.progressBar = ProgressBar(a=self.rowC)
    ...
    def importVolume(self, volume: Volume):
        ...
        self.feedback(msg)
        qId, err = self.app.wdSync.addProceedingsToWikidata(
            wdRecord, write=True, ignoreErrors=False
        )
        if qId is not None:
            _importSpan = self.createWikidataSpan(
                a=self.colD1, wdSync=self.app.wdSync, qId=qId, volume=volume
            )
        else:
    ...
    def createEventItemAndLinkProceedings(self, volume: Volume):
        ...
        (
            ...
            eventQId,
            msg,
        ) = self.app.wdSync.doCreateEventItemAndLinkProceedings(volume, write=True)
    ...
    async def onWikidataButtonClick(self, _msg):
        ...
        try:
            ...
            if qId is not None:
                ...
                _wdSpan = self.createWikidataSpan(
                    a=alert, wdSync=self.app.wdSync, qId=qId, volume=self.volume
                )
            else:
                self.app.feedback(f"error:{err}")
        except Exception as ex:
...
class VolumeSearch:
    ...
    def __init__(
        self, app, volumeInputDiv, volumeDisplay: VolumeDisplay, debug: bool = False
    ):
        """
        constructor
        ...
        self.app = app
        self.debug = debug
        self.app.addMenuLink(
            text="Endpoint",
            icon="web",
            href=self.app.wdSync.endpointConf.website,
            target="_blank",
        )
        self.volumeInput = self.app.createComboBox(
            labelText="Volume",
            a=volumeInputDiv,
            placeholder="Please type here to search ...",
            size="120",
            change=self.onVolumeChange,
        )
...
class VolumeListDisplay(Display):
    ...
    def __init__(self, app, container, debug: bool = False):
        ...
        self.dryRunButton = Switch(
            a=self.colA1, labelText="dry run", checked=self.dryRun, disable=True
        )
        self.dryRunButton.on("input", self.onChangeDryRun)
        self.ignoreErrorsButton = Switch(
            a=self.colA2, labelText="ignore errors", checked=self.ignoreErrors
        )
    ...
    async def onRowSelected(self, msg):
        ...
        if msg.get("selected", False):
            try:
                ...
                volumeId = (
                    int(match.group("volumeNumber")) if match is not None else None
                )
                if volumeId is not None and volumeId in self.app.wdSync.volumesByNumber:
                    ...
                    if proceedingsWikidataId is not None:
                        await self.createEventItemAndLinkProceedings(
                            volume, proceedingsWikidataId, msg
                        )
                    else:
                        Alert(
                            a=self.colA3,
                            text=f"Event not created → Error during creation of proceedings item",
                        )
                else:
                    Alert(
                        a=self.colA3,
                        text=f"Volume for selected row can not be loaded correctly",
                    )
            except Exception as ex:
                self.app.handleException(ex)
    ...
    async def createProceedingsItemFromVolume(self, volume: Volume, msg):
        ...
        if len(wdItems) > 0:
            ...
            qId, errors = self.app.wdSync.addProceedingsToWikidata(
                wdRecord, write=write, ignoreErrors=self.ignoreErrors
            )
            if qId is not None:
                alert = Alert(
                    a=self.colA3, text=f"Proceedings entry for {volume} was created!"
                )
                jp.Br(a=alert)
    ...
    async def createEventItemAndLinkProceedings(
        self, volume: Volume, proceedingsWikidataId: str = None, _msg=None
    ):
        """
        ...
        try:
            ...
            (
                ...
                eventQId,
                msg,
            ) = self.app.wdSync.doCreateEventItemAndLinkProceedings(
...
class WikidataDisplay(Display):
    ...
    async def onReadProceedingsClick(self, _msg):
        ...
        try:
            ...
            self.app.showFeedback(
                f"found {len(proceedingsRecords)} cached wikidata proceedings records"
            )
    ...
    async def onWikidataRefreshButtonClick(self, _msg):
        ...
        try:
            _alert = Alert(
                a=self.app.colA1,
                text="running SPARQL query to retrieve CEUR-WS proceedings from Wikidata ...",
            )
            await self.app.wp.update()
            self.updateWikidata()
            await self.app.wp.update()
    ...
    def updateWikidata(self):
        ...
        _alert = Alert(
            a=self.app.colA1,
            text=f"found {len(wdRecords)} wikidata CEUR-WS proceedings records",
        )
...
class VolumeBrowser(App):
    ...
    def __init__(self, version, args=None):
        ...
        self.addMenuLink(
            text="Wikidata Sync", icon="refresh-circle", href="/wikidatasync"
        )
        self.addMenuLink(text="Settings", icon="cog", href="/settings")
        self.addMenuLink(
            text="github",
            icon="github",
            href="https://github.com/WolfgangFahl/pyCEURmake",
            target="_blank",
        )
        self.addMenuLink(
            text="Documentation",
            icon="file-document",
            href="https://ceur-ws.bitplan.com/index.php/Volume_Browser",
            target="_blank",
        )
        self.addMenuLink(
            text="Source",
            icon="file-code",
            href="https://github.com/WolfgangFahl/pyCEURmake/blob/main/ceurws/volumebrowser.py",
            target="_blank",
        )
        ...
        @jp.app.get(
            "/dblp/volume/{volume_number}/paper/{paper_id}/author", tags=["dblp"]
        )
    ...
    def setupPage(self, header=""):
        header = (
            """<link rel="stylesheet" href="/static/css/md_style_indigo.css">
...
"""
    ...
    async def volumes(self):
        ...
        """
        self.setupRowsAndCols()
        self.volumeListDisplay = VolumeListDisplay(
            self, container=self.rowA, debug=self.debug
        )

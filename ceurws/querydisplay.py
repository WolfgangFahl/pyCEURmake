...
class QueryDisplay:
    ...
    async def onDownloadButtonClick(self, _msg):
        ...
        try:
            alert = Alert(
                a=self.queryBar,
                text=f"Query {self.name} for {self.text} started ... please wait a few seconds",
            )
    ...
    def showDownload(self):
        if getattr(self, "downloadButton", None) is None:
            ...
            for downloadFormat in [
                ...
                self.downloadFormatSelect.add(
                    jp.Option(value=downloadFormat, text=downloadFormat)
                )
    ...
    def showSyntaxHighlightedQuery(self, sparqlQuery, withDownload: bool = True):
        ...
        tryItUrlEncoded = sparqlQuery.getTryItUrl(
            baseurl=self.endpointConf.website, database=self.endpointConf.database
        )
        self.queryDiv.inner_html = queryHigh
        # clear div for try It
        self.queryTryIt.delete_components()
        self.tryItLink = jp.Link(
            href=tryItUrlEncoded,
            text="try it!",
            title="try out with wikidata query service",
            a=self.queryTryIt,
            target="_blank",
        )
